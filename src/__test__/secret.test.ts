...
import { SecretConfig, getSecretConfigFormat } from "../index";
...
describe("test that censor function works with different format name", () => {
  convict.addFormat(
  ...
  it("JSON serialized value or string representation should not contain secret value at all", () => {
    ...
    expect(JSON.stringify(config)).toContain("censored");
  });
});
